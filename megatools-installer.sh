...
megacompver=$(megadl --version | awk {'print $2'} | grep "1.11.0" | sed 's/\.//g')
    ...
    case $mega in
        1)
            if [ ! "$(megadl --version)" ]; then
                ...
                sleep 2
                wget -qO megatools.tar.gz "https://megatools.megous.com/builds/experimental/megatools-1.11.0-git-20200830.tar.gz"
